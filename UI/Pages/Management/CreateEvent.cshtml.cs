...
public class CreateEventModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostPreviewAsync()
    {
        ...
        if (result.IsSuccess)
        {
            TempData["SuccessMessage"] = "Event saved as draft. You are now previewing it.";
            return RedirectToPage("./UpdateEvent", new { id = result.Data }); // Chuyển sang update để có thể preview/chỉnh sửa tiếp
        }

        ModelState.AddModelError(string.Empty, "Error while saving preview: " + result.Message);
        await LoadCatalogsAsync();
        return Page();
    }
